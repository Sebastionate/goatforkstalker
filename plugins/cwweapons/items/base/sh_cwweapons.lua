...
ITEM.functions.Sell = {
	...
	OnRun = function(item)
		...
		sellprice = math.Round((sellprice*(item:GetData("durability",0)/10000))*0.60)
...
ITEM.functions.Value = {
	...
	OnRun = function(item)
		local client = item.player
		local sellprice = item:GetData("RealPrice") or item.price
		sellprice = math.Round((sellprice*(item:GetData("durability",0)/10000))*0.25)
		if item:GetData("durability",0) < 4950 then
			client:Notify("Must be Repaired")
			return false
		end
...
ITEM.functions.RemoveUpgrade = {
	...
	OnCanRun = function(item)
		...
		if item.player:GetChar():HasFlags("A") then
			return (!IsValid(item.entity))
		end 
            ...
            for atcat, dat in pairs(atts) do
                for k, name in pairs(dat.atts) do
					...
					elseif ammotype == "Normal" then
						if (wepon) then
							if name == "am_magnum" or name == "am_matchgrade" or name == "am_slugrounds" or name == "am_flechetterounds" or name == "am_birdshot" or name == "am_zoneloaded" or name == "am_armorpiercing" or name == "am_hollowpoint" then
								client:GiveAmmo(wepon:Clip1(), wepon:GetPrimaryAmmoType(), true)
								wepon:SetClip1(0)
